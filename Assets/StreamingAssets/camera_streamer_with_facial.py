...
from model_common_utils import get_setting_webcam_index
...
def run_pose_script(error_queue):
    ...
    try:
        ...
        try:
            ...
            error_queue.put(f"Pose module error: Error counter during execution")
...
def run_facial_recognition_script(error_queue):
    """Run the facial recognition script as a separate process"""
    try:
        import importlib.util
        import sys
        import os
        ...
        # Get the absolute path to the script
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "facial_recognition_with_gpt_withsocket.py")

        try:
            # Create a module spec
            spec = importlib.util.spec_from_file_location("facial_recognition_with_gpt_withsocket", script_path) #A module specification (or spec) describes how a module is to be imported and loaded.
            pose_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(pose_module)
        
            pose_module.main() # Use the function "main " defined in the module
            error_queue.put(f"emotion recognition module error: Error counter during execution")
            
        except Exception as e:
            print(f"Error running facial expression recognition module: {e}")
            error_queue.put(f"emotion recognition module error: {str(e)}")
            
    except Exception as e:
        print(f"Error in emotion recognition process: {e}")
        error_queue.put(f"Pose module error: {str(e)}")
...
class CameraStreamer:
    def __init__(self, camera_source=0, 
        ...
        self.actual_webcam_resolution_name = "actual_resolution"
        
        # Shared memory objects
        self.low_res_shm = None
        self.high_res_shm = None
        self.actual_webcam_resolution_shm = None
        self.resolution_array = None

        # Pose estimation process
        self.pose_process = None
        # Facial recognition process
        self.facial_process = None
    ...
    def initialize_shared_memory(self):
        ...
        resolution_size = np.dtype(np.float32).itemsize * 2
        ...
        try:
            ...
            try:
                temp = shared_memory.SharedMemory(name=self.actual_webcam_resolution_name, create=False)
                temp.close()
                temp.unlink()
            except FileNotFoundError:
                pass
            ...
            self.actual_webcam_resolution_shm = shared_memory.SharedMemory(
                name=self.actual_webcam_resolution_name, create=True, size=resolution_size)

            self.resolution_array = np.ndarray((2,), dtype=np.float32, buffer=self.actual_webcam_resolution_shm.buf) #buffer arugment use buffer arugment for ndarray, it will reference to the buffer

            width = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
            height = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

            # Store the width and height in shared memory
            self.resolution_array[0] = width  # Store width
            self.resolution_array[1] = height  # Store height
    ...
    def start_facial_recognition(self):
        ...
        try:
            ...
            self.facial_process = Process(target=run_facial_recognition_script, args=(error_queue,))
            self.facial_process.daemon = True
            self.facial_process.start()

            print("Facial recognition process started with PID:", self.facial_process.pid)
        except Exception as e:
            print(f"Error starting facial recognition: {e}")
            error_queue.put(f"Failed to start facial process: {str(e)}")
    ...
    def clean_shared_memory(self):
        ...
        try:
            ...
            if self.actual_webcam_resolution_shm is not None:
                self.actual_webcam_resolution_shm.close()
                self.actual_webcam_resolution_shm.unlink()
    ...
    def stop_pose_estimation(self):
        ...
        if self.pose_process is not None and self.pose_process.is_alive():
            ...
            self.pose_process.join(timeout=1)
    ...
    def stop_facial_recognition(self):
        """Stop the facial recognition process"""
        if self.facial_process is not None and self.facial_process.is_alive():
            print("Terminating facial recognition process...")
            self.facial_process.terminate()
            self.facial_process.join(timeout=1)

            if self.facial_process.is_alive():
                print("Force killing facial recognition process...")
                self.facial_process.kill()
            print("Facial recognition process stopped")
...
def main():
    ...
    streamer = CameraStreamer(
        camera_source=int(get_setting_webcam_index())
    )
    ...
    if args.with_pose:
        ...
        streamer.start_facial_recognition()
